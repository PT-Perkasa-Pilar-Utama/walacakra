...
window.addEventListener("DOMContentLoaded", () => {
  const savedEndpoint =
    localStorage.getItem("apiEndpoint") || getCookie("apiEndpoint");
...
async function simulateProcessing() {
  ...
  if (totalFiles === 0) {
    showAlert(
      "Please upload at least one file before starting the process.",
      "Warning"
    );
    return;
  }
  ...
  const promises = files.map(async (file, index) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error processing ${file.name}: ${response.statusText}`
        );
      }
  ...
  stepDiv.querySelector(
    "#processing-status"
  ).textContent = `✅ All ${totalFiles} files processed successfully`;
